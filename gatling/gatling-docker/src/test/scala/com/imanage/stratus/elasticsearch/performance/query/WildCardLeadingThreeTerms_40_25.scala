...
/*
* Test scenario :
* Wildcard three terms ES query, 40 users, 25 queries each
* */
class WildCardLeadingThreeTerms_40_25 extends Simulation {
  ...
  val scn = scenario("ElasticQueryDirect")
    .repeat(scenarioRepeatCount) {
      exec().feed(feeder1).feed(feeder2).feed(feeder3)
        .feed(custIdFeeder)
        .feed(libIdFeeder)
        .exec(http("three-term-leading-wildcard-search")
          .post(searchPath)
          .headers(headers)
          .body(ElFileBody("com/imanage/stratus/elasticsearch/query/ThreeTermsLeadingWildCardEsQuery.json")))
    }
